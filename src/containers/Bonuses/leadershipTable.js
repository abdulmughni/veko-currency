...
export class LeadershipTable extends Component {
    state = {
        query: {
            $sort: {
                createdAt: -1
    ...
    searchbyUserId = (val) => {
        if (val !== '') {
            let q = this.state.query;
            delete q.$skip;
            this.setState({query: Object.assign({}, q, {description: {$search: val}})}, () => this.letFindTrandsction());
        } else {
            let q = this.state.query;
            delete q.$skip;
...
}), {findTransaction})(LeadershipTable)
